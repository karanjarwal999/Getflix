...
export default function Movies() {
  ...
  const NewPopular = useSelector((store) => store.NewPopular)
  ...
  return (
    <div className={style.movieOuterDiv}>
      {/* append all new and popular list and loader when data is not fetched */}
      <div className={style.popularDiv}>

        {NewPopular.map((element, index) => <MoviesList key={index} ManageModal={ManageModal} MovieData={element} released={element.title === 'upcoming'} />)}
        {NewPopular?.length < 3 ? <Loader /> : null}
          ...
          <ModalBody p={'0px'}>
            ...
            <div className={style.ModalMoreLikeThis} style={{ height: `${expandModal ? 'auto' : '800px'}` }}>
              ...
              <div className={style.moreLikeThisDiv}>
                {NewPopular.map((el) => el.title === ModalData.category ?
